          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              return AppBottomSheet(
                children: const [Center(child: CircularProgressIndicator(color: Color(0xFFD4E99C)))],
              );
            }
            ...
            if (data.isEmpty) {
              return AppBottomSheet(
                children: const [Center(child: Text('Data tidak ditemukan', style: TextStyle(color: Colors.white)))],
              );
            }
            ...
            void addCard(String title, String? value) {
              if (value != null && value.isNotEmpty) {
                if (catChildren.isNotEmpty) catChildren.add(SizedBox(width: 12.w));
            ...
            return AppBottomSheet(
              children: [
                ...
                Author(
                  ...
                  additionalInfo1: authorData['born'] != null ? "Lahir: ${authorData['born']}" : null,
                ...
                CardLittleContainer(
                  child: Column(
                    ...
                    children: [
                      Text('Sumber (Source)', style: TextStyle(color: Colors.white, fontSize: 16.sp, fontWeight: FontWeight.bold)),
                      SizedBox(height: 8.h),
                      Text(data['source']?.toString() ?? '', style: TextStyle(color: Colors.white70, fontSize: 14.sp, height: 1.5)),
                      if (book['page_note'] != null) ...[
                        SizedBox(height: 12.h),
                        Text('Catatan Halaman:', style: TextStyle(color: Colors.white, fontSize: 14.sp, fontWeight: FontWeight.w600)),
                        SizedBox(height: 4.h),
                        Text(book['page_note'].toString(), style: TextStyle(color: Colors.white54, fontSize: 13.sp, height: 1.5)),
                      ]
                    ],
                  ),
                ),
                ...
                if (meaning.isNotEmpty) ...[
                  ...
                  CardLittleContainer(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Overview', style: TextStyle(color: Colors.white, fontSize: 16.sp, fontWeight: FontWeight.bold)),
                        SizedBox(height: 8.h),
                        Text(meaning['overview']?.toString() ?? '', style: TextStyle(color: Colors.white70, fontSize: 14.sp, height: 1.5)),
                        SizedBox(height: 16.h),
                        Text('Key Points:', style: TextStyle(color: Colors.white, fontSize: 16.sp, fontWeight: FontWeight.bold)),
                        SizedBox(height: 12.h),
                        ...(meaning['key_points'] as List<dynamic>? ?? []).map((point) {
                          return Padding(
                            padding: EdgeInsets.only(bottom: 12.h),
                            child: Row(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text('• ', style: TextStyle(color: const Color(0xFFD4E99C), fontSize: 16.sp, fontWeight: FontWeight.bold)),
                                Expanded(child: Text(point.toString(), style: TextStyle(color: Colors.white70, fontSize: 15.sp, height: 1.4))),
                              ],
                            ),
                          );
                        }),
                    ...
                    return Padding(
                      ...
                      child: CardLittleContainer(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(scholar['scholar']?.toString() ?? '', style: TextStyle(color: Colors.white, fontSize: 16.sp, fontWeight: FontWeight.bold)),
                            Text(scholar['work']?.toString() ?? '', style: TextStyle(color: const Color(0xFFD4E99C), fontSize: 14.sp, fontWeight: FontWeight.w500)),
                            SizedBox(height: 12.h),
                            Text(scholar['explanation']?.toString() ?? '', style: TextStyle(color: Colors.white70, fontSize: 14.sp, height: 1.5)),
                          ],
                        ),
                      ),
                    );
                    ...
                    return Padding(
                      ...
                      child: CardLittleContainer(
                         child: Column(
                           crossAxisAlignment: CrossAxisAlignment.start,
                           children: [
                             Text("${ayat['surah']} (${ayat['surah_number']}:${ayat['ayah_number']})", style: TextStyle(color: const Color(0xFFD4E99C), fontSize: 16.sp, fontWeight: FontWeight.bold)),
                             SizedBox(height: 16.h),
                             SizedBox(
                               width: double.infinity,
                               child: Text(
                                 ayat['arabic']?.toString() ?? '',
                                 style: TextStyle(color: Colors.white, fontSize: 24.sp, fontFamily: 'Amiri', height: 1.8),
                                 textAlign: TextAlign.right,
                                 textDirection: TextDirection.rtl,
                               ),
                             ),
                             SizedBox(height: 12.h),
                             Text(ayat['translation']?.toString() ?? '', style: TextStyle(color: Colors.white70, fontSize: 15.sp, height: 1.5)),
                             SizedBox(height: 16.h),
                             Container(
                               ...
                               child: Column(
                                 crossAxisAlignment: CrossAxisAlignment.start,
                                 children: [
                                   Text('Relevansi:', style: TextStyle(color: Colors.white, fontSize: 14.sp, fontWeight: FontWeight.bold)),
                                   SizedBox(height: 4.h),
                                   Text(ayat['relevance']?.toString() ?? '', style: TextStyle(color: Colors.white54, fontSize: 14.sp, height: 1.4)),
                                 ],
                               ),
                             ),
                           ],
